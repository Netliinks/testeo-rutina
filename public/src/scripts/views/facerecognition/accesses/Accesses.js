...
export class Accesses {
    ...
    _schedulePolling() {
        ...
        this._pollTimer = setTimeout(async () => {
            if (!document.getElementById('accesses-grid')) return;
    ...
    async render() {
        ...
        this.datatableContainer.innerHTML = `
            <div class="datatable" id="datatable">
                ...
                <div id="accesses-grid" style="display:grid;grid-template-columns:repeat(6,1fr);gap:12px;min-height:100px;padding:16px 0;"></div>
            </div>
    ...
    async loadAccesses(page) {
        const grid = document.getElementById('accesses-grid');
        ...
        if (!accesses.length) {
            grid.innerHTML = '<p style="color:#808080;font-size:12px;grid-column:1/-1;text-align:center;margin:16px 0;">Sin accesos</p>';
