...
//
import { deleteEntity, getEntityData, registerEntity, setPassword, setUserRole, updateEntity, getUserInfo, sendMail, getFilterEntityData, getFilterEntityCount, postNotificationPush, setFile, createGuardPhoto, getGuardPhotos, getFaceFile, deleteGuardPhotoById, getAllAccessesFromGuardByExternalId } from "../../../endpoints.js";
import { drawTagsIntoTables, inputObserver, inputSelect, CloseDialog, filterDataByHeaderType, getVerifyEmail, getVerifyUsername, pageNumbers, fillBtnPagination, searchUniversalValue, generateFileSimpleXls, sleep, formatearFechaPorZona } from "../../../tools.js";
import { Config } from "../../../Configs.js";
import { tableLayout, UIConvertToSU, UIProgress } from "./Layout.js";
...
export class Guards {
    ...
    load(table, currentPage, data) {
            ...
            for (let i = 0; i < paginatedItems.length; i++) {
                ...
                if((infoPage.showGuards === "N/A" && client?.customer?.id == undefined) || infoPage.showGuards === "THIS" || infoPage.showGuards === "ALL"){
                    ...
                    const entityActions = this.variant === 'photos' ? `
                            ...
                            <button class="button" id="accesses-entity" data-entityId="${client.id}" data-entityName="${client.firstName} ${client.lastName}">
        ...
        if (this.variant === 'photos') {
            ...
            this.editAccesses(this.entityDialogContainer);
        } else {
            this.register();
            this.import();
            this.assignGuard();
            ...
            const readFile = async (file) => {
                ...
                fileReader.addEventListener('load', async (e) => {
                    ...
                    importToBackend.addEventListener('click', async () => {
                        ...
                        const progressTitle = "Importando Guardias";
                        this.showProgress(progressTitle);

                        for (const el of elem) {
                            cont++;
                            this.updateProgress((cont / elem.length) * 100);
                            const userData = JSON.parse(el);
                            document.getElementById('progress-text').innerText = `${cont} / ${elem.length} - ${userData.firstName} ${userData.lastName}`;

                            try {
                                await registerEntity(el, 'User');
                                successCount++;
                                // Pequeña pausa
                                await sleep(50);
                            } catch (error) {
                                failCount++;
                                importErrors.push(`Error en ${userData.username}: ${error.message}`);
                            }
                        }
                        ...
                        if (failCount > 0) {
                            ...
                            this.showProgressError(`Se registraron ${successCount} guardias.`, importErrors.join('\n'));
                        } else {
    ...
    editAccesses(container) {
        const buttons = document.querySelectorAll('#accesses-entity');
        buttons.forEach((btn) => {
            const entityId = btn.dataset.entityid;
            const guardName = btn.dataset.entityname;
            btn.addEventListener('click', () => {
                ...
                container.innerHTML = `
                    <div class="entity_editor" id="entity-editor">
                        <div class="entity_editor_header">
                            <div class="user_info">
                                <div class="avatar"><i class="fa-regular fa-door-open"></i></div>
                                <h1 class="entity_editor_title">Accesos <br><small>${guardName}</small></h1>
                            </div>
                            <button class="btn btn_close_editor" id="close"><i class="fa-solid fa-x"></i></button>
                        </div>
                        <div class="entity_editor_body">
                            <div id="guard-accesses-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;min-height:80px;"></div>
                            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
                                <button id="accesses-prev" style="background:none;border:1px solid #ccc;border-radius:4px;padding:4px 12px;cursor:pointer;font-size:16px;">&#8249;</button>
                                <span id="accesses-page-info" style="font-size:12px;color:#808080;"></span>
                                <button id="accesses-next" style="background:none;border:1px solid #ccc;border-radius:4px;padding:4px 12px;cursor:pointer;font-size:16px;">&#8250;</button>
                            </div>
                        </div>
                    </div>
                `;
                ...
                let accessesPollIntervalId = null;
                const stopAccessesPolling = () => {
                    if (accessesPollIntervalId) {
                        clearInterval(accessesPollIntervalId);
                        accessesPollIntervalId = null;
                    }
                };

                const ACCESSES_PAGE_SIZE = 6;
                let accessesCurrentPage = 0;

                const renderAccessesGrid = async (page, silent = false) => {
                    const grid = document.getElementById('guard-accesses-grid');
                    const pageInfo = document.getElementById('accesses-page-info');
                    const prevBtn = document.getElementById('accesses-prev');
                    const nextBtn = document.getElementById('accesses-next');
                    if (!silent) {
                        grid.innerHTML = '<div style="grid-column:1/-1;display:flex;justify-content:center;align-items:center;min-height:80px;"><span style="display:inline-block;width:28px;height:28px;border:3px solid #e0e0e0;border-top-color:#6F7ADD;border-radius:50%;animation:spin .7s linear infinite;"></span></div>';
                    }
                    const result = await getAllAccessesFromGuardByExternalId(entityId, page, ACCESSES_PAGE_SIZE);
                    const accesses = Array.isArray(result) ? result : (result?.content ?? []);
                    const totalPages = result?.totalPages ?? (accesses.length < ACCESSES_PAGE_SIZE ? page + 1 : page + 2);
                    ...
                    stopAccessesPolling();
                    if (hasPending) {
                        accessesPollIntervalId = setInterval(() => {
                            renderAccessesGrid(accessesCurrentPage, true);
                        }, 5000);
                    }
                };

                renderAccessesGrid(accessesCurrentPage);

                document.getElementById('accesses-prev').addEventListener('click', () => {
                    if (accessesCurrentPage > 0) renderAccessesGrid(--accessesCurrentPage);
                });
    ...
    export = () => {
        ...
        exportUsers.addEventListener('click', async () => {
                    ...
                    <div class="dialog">
                        <div class="dialog_container padding_8">
                            ...
                            <div class="dialog_message padding_8">
                                ...
                                <div class="input_checkbox">
                                    <label><input type="checkbox" class="checkbox" id="check-inactive-users"> Permitir descargar usuarios inactivos</label>
                                </div>
                                <div class="input_checkbox">
                                    <label><input type="checkbox" class="checkbox" id="check-simplified-format"> Descargar formato simplificado</label>
                                </div>
                            </div>
            ...
            `;
                ...
                const _checkInactiveUsers = document.getElementById('check-inactive-users');
                const _checkSimplifiedFormat = document.getElementById('check-simplified-format');
                let onPressed = false;
                exportButton.addEventListener('click', async () => {
                    if(!onPressed){
                        onPressed = true;
                        const exportAllCustomers = _checkAllCustomer.checked;
                        const includeInactiveUsers = _checkInactiveUsers.checked;
                        const simplifiedFormat = _checkSimplifiedFormat.checked;
                        this.dialogContainer.style.display = 'block';
                        this.dialogContainer.innerHTML = `
                        <div class="dialog_content" id="dialog-content">
                            <div class="dialog">
                        ...
                        let rawToExport=(offset)=>{
                            let rawExport = JSON.stringify({
                                "filter": {
                                    "conditions": [
                                        {
                                            "property": `${exportAllCustomers ? 'business.id' : 'customer.id'}`,
                                            "operator": "=",
                                            "value": `${exportAllCustomers ? Config.currentUser.business.id : customerId}`
                                        },
                                        {
                                    ...
                                    for(let y=0; y<array[i].length; y++){
                                        ...
                                        if (simplifiedFormat) {
                                            ...
                                            users.push({
                                                ...
                                                "Apellido 1":`${user["lastName"] ?? ''}`,
                                                "Apellido 2":`${user["secondLastName"] ?? ''}`,
                                                "Cédula":user["dni"] ?? '',
                                            });
                                        }
                                    }
            ...
            const readFile = async (file) => {
                ...
                fileReader.addEventListener('load', (e) => {
                    ...
                    importToBackend.addEventListener('click', async () => {
                        ...
                        const dialogContainer = document.getElementById('app-dialogs');
                        const progressTitle = "Importando Asignaciones";
                        this.showProgress(progressTitle);
                        ...
                        for (const el of elem) {
                            cont++;
                            this.updateProgress((cont / elem.length) * 100);
                            document.getElementById('progress-text').innerText = `${cont} / ${elem.length} - ${el["username"]}`;
                            ...
                            try {
                                ...
                                if (data != undefined) {
                                    if (el["customer_id"] && el["customer_id"].trim() !== "" && data.customer.id != el["customer_id"]) {
                                        // Verificar si la empresa destino existe mediante conteo (más eficiente)
                                        const rawCheck = JSON.stringify({
                                            "filter": {
                                                "conditions": [{ "property": "id", "operator": "=", "value": el["customer_id"] }]
                                            }
                                        });
                                        ...
                                        if (count === 0) {
                                            failCount++;
                                            importErrors.push(`Empresa no existe: ${el["customer_id"]} (Guardia: ${el["username"]})`);
                                            continue;
                                        }
                                        ...
                                        if (existUserRoutine == undefined) {
                                            failCount++;
                                            importErrors.push(`Error buscando rutina: ${el["username"]}`);
                                        } else {
                                            if (existUserRoutine.length > 0) {
                                                for (let i = 0; i < existUserRoutine.length; i++) {
                                                    await deleteEntity('RoutineUser', existUserRoutine[i].id);
                                                }
                                            }

                                            const rawAndroid = JSON.stringify({
                                                "user": { "id": `${entityId}` },
                                                "model": '#NEWMOBILEADD'
                                            });
                                            await registerEntity(rawAndroid, 'AndroidLogin');
                                            await update(el["customer_id"], entityId);

                                            let pushMsg = JSON.stringify({
                                                "title": "Cambio de empresa",
                                                "body": `Ha sido removido de la empresa ${data?.customer?.name ?? ''}, por favor reinicie la aplicación ahora.`,
                                                "tokenUser": data['token'],
                                                "type": "routine-info"
                                            });
                                            postNotificationPush(pushMsg);
                                            successCount++;
                                        }
                                    } else {
                                        successCount++; // No hay cambio o ID vacío (saltado)
                                    }
                                } else {
                                    failCount++;
                                    importErrors.push(`Guardia no encontrado: ${el["username"]}`);
                                }
                            } catch (error) {
                                failCount++;
                                importErrors.push(`Error en ${el["username"]}: ${error.message}`);
                            }
                        }
                        ...
                        new Guards().render(infoPage.offset, infoPage.currentPage, '', infoPage.showGuards);
                    });
                });
            }
            const closeButton = document.getElementById('close');
                    ...
                    <div class="dialog_content" id="dialog-content">
                        <div class="dialog">
                            <div class="dialog_container padding_8">
                                ...
                                <div class="dialog_message padding_8" style="max-height: 70vh; overflow-y: auto;">
                                    <div class="datatable_tools">
